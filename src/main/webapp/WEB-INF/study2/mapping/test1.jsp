...
<div class="container">
  ...
  <div id="demo3">
    <div>전송된 today : ${daytime}</div>
    <c:set var="dayjsp" value="${fn:substring(daytime,0,10)}"/> 
    <c:set var="timejsp" value="${fn:substring(daytime,11,19)}"/> 
    <div>오늘 날짜 : ${day}</div>
    <div>오늘 날짜 : ${dayjsp}  -- jsp</div>
    <div>현재시간 : ${time}</div>
    <div>현재시간 : ${timejsp}  -- jsp</div> 
  </div>
</div>
<p><br/></p>
